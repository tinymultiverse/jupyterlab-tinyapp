...
APP_PREVIEW_URL = os.getenv('APP_PREVIEW_URL', f'http://127.0.0.1:8005')
...
class PreviewHandler(CustomAPIHandler):
    ...
    async def post(self):
        ...
        with await execution_lock.acquire():
            ...
            self.finish(json.dumps({
                'data': {
                    'appURL': f'{APP_PREVIEW_URL}{JUPYTER_BASE_URL}{APP_PREVIEW_BASE_URL}'
                }
            }))
...
class PingHandler(CustomAPIHandler):
    ...
    async def get(self):
        ...
        ok = False
        api_route = f'http://127.0.0.1:{APP_PREVIEW_PORT}{JUPYTER_BASE_URL}{APP_PREVIEW_BASE_URL}{HEALTH_CHECK}'
